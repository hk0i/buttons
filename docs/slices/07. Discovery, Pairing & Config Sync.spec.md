...
9. **Single-connection guard is a `Mutex<bool>` (or an `AtomicBool`) in
...
6. [ ] Stale/reused one-time token (generate a new QR on desktop, then scan
       ...
       config received.
7. [ ] A second mobile device attempting to connect while the first is
       active gets rejected with a reason, doesn't disrupt the first
       device's session.
8. [ ] `cargo build` (desktop/src-tauri) succeeds with the new deps.
9. [ ] `npm run check` (desktop) passes with no new errors.
10. [ ] iOS target builds and runs on a real device (Simulator can't
       exercise the mDNS+camera round trip — see Implementation Notes #9).
11. [ ] `protocol/verify-kotlin`'s existing build still compiles
