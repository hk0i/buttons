...
6. [ ] Stale/reused one-time token (generate a new QR on desktop, then scan
       ...
       config received.
7. [ ] Simulate a ghost connection — airplane mode or walking out of Wi-Fi
...
8. [ ] An unauthenticated connection attempt (garbage/no `PairRequest`)
       while a real session is active does **not** disrupt that session —
       proves eviction only happens post-authentication, not on raw
       connect.
9. [ ] `cargo build` (desktop/src-tauri) succeeds with the new deps.
10. [ ] `npm run check` (desktop) passes with no new errors.
11. [ ] iOS target builds and runs on a real device (Simulator can't
       exercise the mDNS+camera round trip — see Implementation Notes #11).
12. [ ] `protocol/verify-kotlin`'s existing build still compiles
