...
7. [ ] One-time token actually expires on its own timer, not just on
       regeneration: generate a QR, wait past the 90s TTL **without**
       requesting a new one, then scan it → rejected. (Distinct from item
       6 — that one tests invalidation-by-replacement; this tests the
       clock actually runs.)
