...
5. **`PairingView`'s landing copy — three states from two axes** (credential
   ...
   generic fallback:
   1. **No credential** (fresh install / never paired) — "Scan your
      desktop's QR code to pair." (`.idle`, unchanged, today's copy). If
      this state ever needs its own "searching" variant (it doesn't today
      — nothing auto-browses before a first pair), the sentence is
      "Looking for Buttons Desktop on your network…" — any instance, no
      name to show since none is known yet. This state's real home is a
      future device picker (`07b`'s debug list, or roadmap step 11's
      manual-entry slice), not the normal paired-reconnect flow.
   2. **Credential held, no match yet** (`.searching`) — "Reconnecting to
      `<device_name>`…" — a specific `device_id` is being sought, not a
      space of candidates; "reconnecting" says that, "looking"/"searching"
      doesn't.
   3. **Credential held, matched, handshake in flight** (new `.connecting`)
