...
async fn apply_profile_switch(
    ...
    // 3. new — no-op if already active: without this, focus_watcher's
    // caller would need its own copy of this check (Implementation Notes
    // #6), and a redundant manual request (slice 10) would keep rewriting
    // buttons.json and re-broadcasting for nothing
