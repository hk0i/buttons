...
1. `buttons.proto`: `Profile` gets `repeated string associated_app_ids = 4;`
   — platform-native identifiers of the app(s) this Profile auto-switches
   for. `repeated`, not a scalar, from the start: EDD § 5.5 Desktop
   component #5 ("Profile export/import to a plain JSON file") makes this
   field's shape part of a user-facing, shareable export format the day
   this slice ships — reshaping scalar→repeated later would break that
   format and every generated-language binding reading it, the same
   non-additive-change problem 09a's `repeated StateChange` was decided
   against up front for. Deliberately generic, not `bundle_id(s)`: macOS
   stores an `NSRunningApplication` bundle identifier (e.g.
   `com.google.Chrome`) this slice, a Windows executable name (e.g.
   `chrome.exe`) is a second entry in the same list once that watcher
   lands — no rename, no reinterpretation, purely additive. No `platform`
   tag alongside each id: `com.aseprite.Aseprite` and `aseprite.exe` don't
   collide, so the platform is already implicit in the id's own shape: a
   lookup is just "does any entry match the currently-focused app id,"
   with no enum/tag needed (and proto3 canonical JSON would render a
   platform enum as `"PLATFORM_WINDOWS"`, fighting EDD § 5.3's
   human-readable/diffable export goal).
...
3. Desktop Rust: `focus_watcher.rs`, `#[cfg(target_os = "macos")]` —
   ...
   focus event), then looks up a `Profile` whose `associated_app_ids`
   contains the newly-focused app's bundle id.
...
6. Desktop Svelte: `ProfileSwitcher.svelte` gets an app-association picker
   ...
   association(s) shown, "None" clears them. This slice's picker sets a
   **single** entry (replaces `associated_app_ids` with a one-element
   list) — the schema's `repeated` shape exists for Windows to add a
   second entry later, and for a possible future multi-app-per-Profile
   case (Scope → Out #5), not because this slice's UI manages a list.
...
1. **Windows watcher.** Same feature, different native API (no bundle-id
   ...
   receive it as a second list entry, not a reinterpretation of the first
   — a Windows-configured Profile just appends its `.exe` name alongside
   whatever macOS bundle id is already there, no migration of existing
   exported configs. Sequencing reason, not a technical one: iOS App
   Store readiness (step 11a) is the current priority — see roadmap
   Testing & Rollout Plan step 11a's own "gates step 12" framing, same
   logic applied one level earlier here.
...
6. **One Profile associated with multiple different apps** (e.g. an "Art"
   Profile for both Aseprite and Photoshop) — the `repeated` schema
   permits it and a future slice could add list-management UI for it, but
   this slice's single-entry picker (Scope → In #6) never produces it.
...
1. `protocol/buttons.proto` — modify — add `associated_app_ids` to
   `Profile`, tag 4.
...
3. `desktop/src-tauri/src/config.rs` — modify — `Profile` struct gets
   `associated_app_ids: Vec<String>`.
4. `desktop/src-tauri/src/focus_watcher.rs` — create — macOS
   `NSWorkspace` watcher, debounced, `cfg(target_os = "macos")`-gated;
   no-op stub (or absent module) on other targets.
5. `desktop/src-tauri/src/server.rs` — modify — extract shared
...
6. `desktop/src-tauri/src/lib.rs` — modify — create `ProfileSwitchTx` at
...
7. `desktop/src/lib/stores/config.svelte.ts` — modify — `Profile` type
   gains `associatedAppIds: string[]`; store exposes a setter that
   replaces the list with a single-element one (Scope → In #6).
8. `desktop/src/lib/components/ProfileSwitcher.svelte` — modify —
   app-association picker per Profile.
9. `ios/Buttons/Networking/Connection.swift` — modify — `.profileSwitch`
   case in `handle(envelope:)` applies the id (replacing the current
   no-op arm at line 345).
10. `ios/Buttons/Generated/buttons.pb.swift` — regenerate (schema change,
    unlike slice 10 which only touched `wire.pb.swift`).
11. `protocol/wire.proto` — modify — comment only: drop the "M→D only this
...
sequenceDiagram
  ...
  W->>W: match bundle id against Config.profiles[].associated_app_ids
...
message Profile {
  ...
  // 1. new — platform-native app identifiers (macOS: bundle id; Windows
  // (later): executable name), one entry per platform this Profile
  // auto-switches for. Empty = no auto-switch association.
  repeated string associated_app_ids = 4;
}
```

1. `repeated`, not `optional string` — decided for the same reason 09a's
   `StatePush.changes` was `repeated` from day one (Scope → In #1):
   `Profile` export/import (EDD § 5.5 Desktop component #5) makes this a
   shareable file format immediately, and scalar→repeated is a non-
   additive reshape once that format is in use.
...
1. **Why `objc2`/`objc2-app-kit` and not the older `cocoa`/`objc` crates**
...
2. **Debounce lives in `focus_watcher.rs`, not in the shared switch
...
3. **The watcher never touches the wire or `ConfigDirtyTx` directly** —
...
4. **Windows deferral is a sequencing call, not a technical blocker** — the
   ...
   the running-apps picker's Windows equivalent are missing. Picking this
   back up is additive, not a rework.
...
Additive (new optional `Profile` field, new broadcast channel, new
macOS-only module, comment-only `wire.proto` change) — safe to `git revert`
