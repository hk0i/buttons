...
1. `buttons.proto`: `Profile` gets `optional string associated_app_id = 4;`
   — the platform-native identifier of the app this Profile auto-switches
   for. Deliberately generic, not `bundle_id`: macOS stores an
   `NSRunningApplication` bundle identifier (e.g. `com.google.Chrome`) this
   slice, but the field is designed to hold a Windows executable name
   (e.g. `chrome.exe`) unchanged when that watcher lands — no rename, no
   schema migration, additive later same as everything else in this
   protocol.
...
3. Desktop Rust: `focus_watcher.rs`, `#[cfg(target_os = "macos")]` —
   ...
   focus event), then looks up a `Profile` whose `associated_app_id`
   matches the newly-focused app's bundle id.
...
6. Desktop Svelte: `ProfileSwitcher.svelte` gets an app-association picker
   ...
   association shown, "None" clears it. Exact placement (inline per-row vs.
   a small edit affordance) is an implementation-time call.
...
1. **Windows watcher.** Same feature, different native API (no bundle-id
   ...
   receive it with no rename. Sequencing reason, not a technical one: iOS
   App Store readiness (step 11a) is the current priority — see roadmap
...
5. **Two Profiles claiming the same `associated_app_id`.** Not detected or
   prevented; whichever the watcher's lookup finds first wins,
   undefined-but-harmless. No UI validation against it this slice.
6. **Any change to slice 10's manual M→D switch path.** Unaffected — still
...
1. `protocol/buttons.proto` — modify — add `associated_app_id` to
   `Profile`, tag 4.
...
3. `desktop/src-tauri/src/config.rs` — modify — `Profile` struct gets
   `associated_app_id: Option<String>`.
...
7. `desktop/src/lib/stores/config.svelte.ts` — modify — `Profile` type
   gains `associatedAppId`; store exposes a setter.
...
sequenceDiagram
  ...
  W->>W: match bundle id against Config.profiles[].associated_app_id
...
message Profile {
  ...
  // 1. new — platform-native app identifier (macOS: bundle id; Windows
  // (later): executable name). Unset = no auto-switch association.
  optional string associated_app_id = 4;
}
```
