...
3. `desktop/src-tauri/src/config.rs` — modify — new `Platform` enum
   (`MacOs` only this slice, closed set, application-side only —
   Implementation Notes #1; `Windows`/`Linux` variants wait for the code
   that constructs them, own future slices); `Profile` struct gets
...
```rust
// 1. new — config.rs
/// A platform a Profile can auto-switch for.
