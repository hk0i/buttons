...
7. `desktop/src-tauri/src/lib.rs` — modify — create `ProfileSwitchTx` at
   ...
   app setup, wired to the shared switch function; new Tauri command
...
6. [ ] Setting Profile B's macOS association, then hand-editing
   ...
   unchanged. Confirms Scope → In #6 / Files to Touch #8's "writes only
   the `\"macos\"` key" claim, the one this slice's Windows-later story
   (Scope → Out #1) actually depends on.
