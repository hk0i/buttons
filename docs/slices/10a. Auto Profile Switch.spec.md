...
6. Desktop Svelte: `ProfileSwitcher.svelte` gets an app-association picker
   ...
   `set_app_association`. Rendered only when `auto_switch_supported`
...
7. `desktop/src-tauri/src/lib.rs` — modify — create `ProfileSwitchTx` at
   ...
   `list_running_apps` (Scope → In #2), `auto_switch_supported`,
...
9. `desktop/src/lib/components/ProfileSwitcher.svelte` — modify —
   ...
   `auto_switch_supported` returns `true`; reads via
...
9. **The app-association read/write moved entirely to Rust after an
   ...
   commands (`auto_switch_supported`, `get_app_association`,
