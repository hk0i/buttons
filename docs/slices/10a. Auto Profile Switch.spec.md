...
2. **Linux watcher.** No standard cross-compositor Wayland API exists (a
   ...
   the same thing (one OS, one API, one id shape), Linux isn't one
   namespace — X11's `WM_CLASS`/process-name and any future Wayland
   integration (compositor-specific: wlroots' foreign-toplevel protocol,
   a GNOME Shell extension, a KWin script) produce incompatible id shapes
   from different, unrelated APIs. The map key should likely name the
   actual backend (`"x11"`, not `"linux"`) so a user's X11→Wayland
   migration — increasingly the Linux default — shows up as "needs a new
   association" rather than a silently-broken old one. Don't decide this
   for real until the slice that builds it.
