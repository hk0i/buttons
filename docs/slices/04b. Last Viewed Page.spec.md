...
- **Fallback chain** (stale entries are harmless, so no pruning needed):
  ...
  `pages[0]?.id ?? null`.
- **EDD §5.2 + Open Questions** — *already revised alongside this spec
...
- **EDD §5.3 note** *(implementation task)* — add a bullet to
...
- **Pruning stale `localStorage` entries** on `deleteProfile` / `removePage`
  ...
  prune then — not now.
- **Putting navigation state on the wire.** This slice's persistence is
...
- **Anything about the native iOS / Android apps.** They don't persist a
...
- `/docs/01. Architecture Overview.edd.md` — **modify** — §5.3 boundary
