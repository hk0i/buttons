...
- **Native shells won't reuse this.** `ios/` / `android/` view state belongs
  in `UserDefaults` / `SharedPreferences` per platform, mirroring how §5.3
  already splits auth-token storage. Not shared code, by design.
