...
1. [x] Park on Page 3 of a Profile, Cmd-Q, relaunch → app opens on Page 3,
       not Page 1. `folderStack` is empty (folders not restored).
2. [x] Switch to another Profile that was last on Page 2, then back → each
       returns to its own last page.
3. [x] A Profile never navigated away from Page 1 still opens on Page 1.
4. [x] Delete the page you were parked on → next launch falls back to Page 1
       (no error, no blank grid).
5. [x] Manually clear `localStorage` → app launches on Page 1 for every
       Profile, no error.
6. [x] `npm run check` passes with no new errors.
7. [x] EDD §5.3 records nav state as device-local, outside `Config`.
