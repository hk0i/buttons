...
**In:**
- **`navState.ts`** — a tiny device-local read/write helper over
...
- **Restore on launch** — `configStore.load()` sets `currentPageId` to the
  active Profile's stored last page instead of always `pages[0]`.
- **Restore on Profile switch** — `configStore.switchProfile()` restores the
...
- **Record on navigation** — `configStore.selectPage()` writes the new page
...
- **Fallback chain** (stale entries are harmless, so no pruning needed):
  stored id → still present in `activeProfile.pages`? → else
  `pages[0]?.id ?? null`.
- **EDD note** — add a bullet to `01. Architecture Overview.edd.md` §5.3
  that **draws the storage boundary**, not just records this choice:
...
**Out:**
- **Persisting folder depth.** `folderStack` still resets to `[]` on launch
...
- **Pruning stale `localStorage` entries** on `deleteProfile` / `removePage`
  ...
  prune then — not now.
- **Syncing last-viewed page across devices** — the entire point is that it
  stays local. Not a `config_sync` field, not a `profile_switch` payload.
- **A general device-local settings store / framework.** This is one helper
  for one `Record`. If a second device-local setting shows up later
  (window bounds, per-device theme override), generalize then.
- **Migrating existing `Config` persistence** or touching the Rust
  `get_config` / `save_config` path at all.
- Light-mode styling — still deferred project-wide.
...
- `/desktop/src/lib/stores/navState.ts` — **create** — plain `.ts` (no
  ...
  empty.
- `/desktop/src/lib/stores/config.svelte.ts` — **modify** —
  - add a private `pageIdFor(profile: Profile | null): string | null`
    implementing the fallback chain (reads `getLastPageId`).
  - `load()`: `this.currentPageId = this.pageIdFor(this.activeProfile);`
    (still `this.folderStack = [];`).
  - `switchProfile()`: `this.currentPageId = this.pageIdFor(this.activeProfile);`
    in place of the current `?.pages[0]?.id ?? null`.
  - `selectPage(pageId)`: after setting `currentPageId`, call
    `setLastPageId(this.config.activeProfileId, pageId)` when
    `this.config` is present. Stays sync.
- `/docs/01. Architecture Overview.edd.md` — **modify** — §5.3 bullet (see
  Scope).
...
- **Why `localStorage` and not a Rust-backed file:** verified 2026-09-10
...
- **One key, not a key per profile** — a single `Record` is one parse, one
  write, and trivially inspectable in devtools.
- **Dev and prod are different origins.** `tauri dev` serves the frontend
  ...
  so it isn't chased.
- **Rust can't read `localStorage`.** Fine here — nothing Rust-side needs
  the current page — but it's the constraint that would force a
  Rust-backed file if that ever changes.
- **The native apps won't reuse this.** `ios/` / `android/` are full native
...
- **`pageIdFor` is the single source of the fallback chain** — both `load()`
  and `switchProfile()` call it, so "stored → valid? → page 1" lives once.
- **`selectPage` is the only write point.** Confirm during implementation
...
- **Do not write from `switchProfile` / `load`.** Restoring a page is not a
  user choice; only an explicit `selectPage()` records one. Writing back the
  restored value would be harmless but muddies intent.
- Keep `navState.ts` dumb: no knowledge of `configStore`, profiles, or
  pages beyond the two string ids it's handed.

## Definition of Done

- [ ] Park on Page 3 of a Profile, Cmd-Q, relaunch → app opens on Page 3,
      not Page 1. `folderStack` is empty (folders not restored).
- [ ] Switch to another Profile that was last on Page 2, then back → each
      returns to its own last page.
- [ ] A Profile never navigated away from Page 1 still opens on Page 1.
- [ ] Delete the page you were parked on → next launch falls back to Page 1
      (no error, no blank grid).
- [ ] Manually clear `localStorage` → app launches on Page 1 for every
      Profile, no error.
- [ ] `npm run check` passes with no new errors.
- [ ] EDD §5.3 records nav state as device-local, outside `Config`.
