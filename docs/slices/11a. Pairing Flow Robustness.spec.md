...
12. **Resolved 2026-09-20: the picker is never shown while connected —
    ...
    the picker deliberately — no manual disconnect existed at all. Added
    `DesktopConnection.disconnectToSwitchDevices()` (sets
    `wasDisconnectedIntentionally`, then calls the existing
    `disconnect()`) wired to a "Switch Device" button in the connected
    chrome. The existing drop-recovery observer
    (`.onChange(of: connection.isConnected)`, added 07d/09b) checks and
    consumes that flag — an intentional switch must not immediately
    auto-reconnect to the device just left, which is exactly what the
    existing involuntary-drop recovery would otherwise do. A related,
    explicitly **separate** future scope item: an auto-reconnect-on/off
    preference ("remember me"-style) — not needed to resolve this.
...
8. **Manual disconnect — Design decision 12.** New
   `DesktopConnection.disconnectToSwitchDevices()`, wrapping the
   existing `disconnect()` with `wasDisconnectedIntentionally` (a
   `private(set)` flag, cleared via `acknowledgeIntentionalDisconnect()`).
   A "Switch Device" button in `ButtonsApp.swift`'s connected chrome
   (`RootView`'s `safeAreaInset`, now gated on `connection.isConnected`
   rather than `configSync`/profile count) calls it. The existing
   `.onChange(of: connection.isConnected)` drop-recovery observer checks
   and consumes the flag before calling `attemptAutoReconnect()` — an
   intentional switch must not immediately reconnect to the device just
   left.
...
3. `ios/Buttons/Networking/Connection.swift` — modify — `PairResult`
   carries `deviceName` alongside `authToken`; `disconnectToSwitchDevices()`
   / `wasDisconnectedIntentionally` / `acknowledgeIntentionalDisconnect()`
   for manual disconnect (Design decision 12 / Scope In item 8).
...
5. `ios/Buttons/ButtonsApp.swift` — modify — `PairingSession`
   ...
   construction, "Switch Device" button, drop-recovery observer checks
   `wasDisconnectedIntentionally`.
...
12. [ ] An involuntary drop (kill the desktop app, don't tap "Switch
        Device") still auto-reconnects as before — the intentional-drop
        flag doesn't accidentally suppress real recovery.
...
#     fires as before, unaffected by the intentional-disconnect flag
