...
12. **Resolved 2026-09-20: the picker is never shown while connected —
    ...
    2. **Replaced with a callback fired at the point of detection,
       ...
       `receiveLoop`'s failure branch — specifically the case that
    ...
    4. **Found the hard way: `disconnect()` cancelling the socket still
       ...
       resolves `receiveLoop`'s pending `receive` with a failure — the
       exact branch that calls `onConnectionLost`, so tapping "Switch
       Device" reconnected right back. Fixed by binding `receiveLoop` to
...
8. **Manual disconnect — Design decision 12.** A "Switch Device" button
   ...
   and invoked only from `receiveLoop`'s already-established-connection
...
3. `ios/Buttons/Networking/Connection.swift` — modify — `PairResult`
   ...
   `onConnectionLost: (() -> Void)?`, invoked only from `receiveLoop`'s
...
12. [ ] An involuntary drop (kill the desktop app, don't tap "Switch
        ...
        fires from `receiveLoop`'s failure branch same as it always
        would have.
