...
8. `desktop/src-tauri/src/mdns.rs` — new, 2026-09-22 (Design decision 14)
   ...
   registered fullname; `start()`/`rename()`.
...
10. `desktop/src-tauri/src/lib.rs` — modify — `mdns::Advertisement`
    ...
    `mdns.rename(...)` with it.
11. `docs/slices/07c. Desktop Device Name.spec.md` — modify, 2026-09-22 —
    Out item 1 marked superseded.
12. `docs/slices/11a. Pairing Flow Robustness.spec.md` — this file.
...
15. [ ] An undiscovered/never-paired desktop's row shows its live Bonjour
        ...
        Device" or a raw ID.
16. [ ] Renaming the desktop while the app is running (via its own
        settings UI) is reflected immediately in `dns-sd -B
        _buttons._tcp` / a live picker — no restart required.
