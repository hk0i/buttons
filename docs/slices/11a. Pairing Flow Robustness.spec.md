...
8. **`StoredPairing` gains `deviceName: String?` from its first new-shape
   draft — absorbing `07c`'s iOS-side scope, not layering it on later.**
   `07c. Desktop Device Name.spec.md` split 2026-09-20: its wire +
   desktop half (`PairResponse.device_name`, `device.json` persistence,
   settings-UI rename field) ships standalone, independent of this
   slice. Its iOS half doesn't get built against the *current*
   ...
   shipped. A raw `deviceId` UUID in the picker would defeat this
   slice's own dev-workflow motivation (telling a real desktop apart
...
layer above the protocol boundary, not in it. Shows the constructor
injection (`SecretStore` defaults to the real `KeychainStore`, a fake
goes in for tests — Design decision 11), and a "getting/setting auth"
...
struct StoredPairing: Equatable, Codable {
    ...
    let deviceName: String?  // nil if the desktop predates 07c's wire field
}
...
`name` field. A "Pair new…" row has no name yet — nothing's been paired
to source one from — until the pair completes:
...
`session.connect(to:)` (wraps the existing `reconnect(stored:endpoint:)`
with `setActive` + a refresh — see the `PairingSession` sketch above),
"Pair new…" rows route to the existing QR scan flow.
...
2. `PairingStore` (`Pairing.swift`) refactored onto `SecretStore`: drops
   ...
   `deviceName: String?` from its first draft (Design decision 8,
   absorbing `07c`'s iOS half) — packed with `authToken` into one JSON
   value per device key. No default in `PairingSession`'s initializer
...
3. `PairingSession` takes ownership of `DesktopDiscovery` (Design
   ...
   PairedDeviceRow)` wraps `reconnect(stored:endpoint:)` +
   `setActive(deviceId:)` + a refresh, so `PairingView` never calls
   `PairingStore` directly.
4. `ios/Buttons/Networking/Connection.swift`: `PairResult.success` gains
   `deviceName: String?` alongside `authToken` (07c's Files to Touch #8,
   read from `PairResponse.device_name` once the wire/desktop half has
   shipped — depends on that, doesn't rebuild it).
...
6. `ios/Buttons/Views/PairingView.swift` — modify — device list UI reading
   `session.deviceRows`, showing `name ?? deviceId`, "Connect"/"Pair
   new…" actions, drop the `discovery:` property; preview provider
   updated to match.
