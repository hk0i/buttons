...
async fn handle_client_message(
    ...
    dirty_tx: &crate::ConfigDirtyTx, // 1. new — reuses 09c's channel
    app: &tauri::AppHandle,
    ...
    match envelope.message {
        ...
        Some(buttons::envelope::Message::ProfileSwitch(switch)) => {
            ...
            None // no reply envelope — see Scope → Out #4
        }
        _ => None, // no other client-initiated message this slice
    }
}
...
async fn handle_profile_switch(
    ...
    let _ = dirty_tx.send(config); // feeds 09c's debounce → config_sync to mobile
...
if (!this.unlisten) {
  this.unlisten = await listen<string>("active-profile-changed", (event) => {
    this.applyActiveProfile(event.payload); // 2. new — shared with switchProfile()
  });
}
```
