...
6. `ios/Buttons/Networking/Envelope+Send.swift` — modify — add
   `.profileSwitch(to:)` factory, alongside `.pairRequest(token:)`/
   `.buttonPress(_:)` (added by the boilerplate sweep that lands before
   this slice — see Implementation Notes #4).
7. `ios/Buttons/Networking/Connection.swift` — modify — add
   `requestProfileSwitch(to:)`.
8. `ios/Buttons/ButtonsApp.swift` — modify — Profile switcher UI on
   `RootView`, wired to `connection.requestProfileSwitch(to:)`.
9. `ios/Buttons/Generated/wire.pb.swift` — regenerate (command below).
...
**Mobile (`Envelope+Send.swift`, `Connection.swift`):**

```swift
// 1. new — Envelope+Send.swift, alongside .pairRequest/.buttonPress
...
1. Same factory shape `.pairRequest(token:)`/`.buttonPress(_:)` already
   use post-sweep (Implementation Notes #4) — `make(_:)` stamps
   `protocol_version` once, this factory only names what's distinct.
2. Empty-id guard mirrors `pressButton`'s own (same file): a real id is
   never empty (desktop-generated), but a corrupted persisted
   `buttons.json` re-serving the same bad id on every relaunch would turn
   a crash here into a crash loop — a silent no-op instead.
...
4. **`Envelope+Send.swift` (`make(_:)` + `.pairRequest`/`.buttonPress`
   factories) is a prerequisite, not part of this slice** — a separate
   `[ios]` sweep replacing `Connection.swift`'s var-then-assign envelope
   construction, landed 2026-09-16 ahead of this slice specifically so
   `requestProfileSwitch` gets written once, in its final shape, instead
   of matching the old pattern and being rewritten right after.

## Definition of Done

1. [x] With mobile and desktop both showing Profile A, tapping Profile B in
...
2. [x] The same tap updates mobile's own `DeckView` to Profile B's pages,
   within roughly one debounce period — no manual refresh, no reconnect.
3. [x] Switching to a Profile whose current mobile page id doesn't exist in
...
4. [x] A `profile_switch` naming a Profile id that doesn't exist in
   `Config` is dropped server-side, doesn't crash the connection, and
   doesn't change `active_profile_id` — verified 2026-09-16 via
   `websocat` as a fake mobile client, paired against the real running
   desktop using the persisted `auth_token` from `device.json` (no QR
   needed — see PAIRING.md's reconnect-token validator path). Sent a
   bogus id: `activeProfileId` in `buttons.json` unchanged. Sent a real
   id right after, same connection: it changed correctly, proving the
   bogus one didn't kill the connection or handler. (Desktop console's
   own drop log line not captured here — confirm by eye if you want the
   exact log text; the file-state evidence above already covers the
   DoD's substance.)
5. [x] `wire.proto` changes compile in all three generated languages
   (schema-lock check, same as slice 08/09a): `android/gradlew
   --project-dir protocol/verify-kotlin test`. **Amended 2026-09-16:**
   the JDK 19/Kotlin-daemon toolchain break 09/09a's own DoD 1 carried no
   longer reproduces — ran clean, `RoundTripTest`: 3 tests, 0 failures
   (`verify-kotlin/build/test-results/test/TEST-buttons.RoundTripTest.xml`).
   Backlog memory's "JDK19/Kotlin-daemon toolchain break" entry is stale;
   updated.
6. [x] `protoc --swift_out=…` regen produces the expected `wire.pb.swift`
   diff; `buttons.pb.swift` has **no** diff. Verified 2026-09-16: zero
   diff on either file — the committed `wire.pb.swift` already matches
   what `protoc` generates from the current `.proto` (last regenerated in
   this slice's own step 1, nothing since has touched the schema).
