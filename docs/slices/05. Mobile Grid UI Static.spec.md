...
10. **Locking Page-swipe while a folder is open is a data change, not a
    ...
    go, no gesture race. **Known risk, not yet confirmed either way:**
    `PageGrid`'s `folderStack` is `@State`, keyed to its Page's view
    identity; shrinking `ForEach`'s collection to one element and back
    might tear down and recreate the surviving `PageGrid`, resetting
    `folderStack` to `[]` the instant a folder is entered. DoD item 4 below
    must explicitly confirm this doesn't happen. If it does, the fix is
    lifting `folderStack` into `ContentView` (an `@Observable` rule
    criterion-1 case — state shared between parent and child). Fallback if
    that also proves awkward: hand-roll the pager with
    `ScrollView(.horizontal) + .scrollTargetBehavior(.paging)` (iOS 17+),
    which has a real `.scrollDisabled(_:)`, at the cost of re-implementing
    page-indicator dots.
...
4. [ ] Tapping a folder button drills into its nested grid, which shows a
       ...
       one visible Page doesn't reset `PageGrid`'s `folderStack`.
