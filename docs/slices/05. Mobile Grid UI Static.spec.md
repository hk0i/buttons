...
- **Bug found + fixed during the walkthrough:** initial swipe-lock used
...
- **Bug found + fixed:** page-indicator dots were invisible (unselected-gray
  on transparent, over a white screen). Fixed with
  `.indexViewStyle(.page(backgroundDisplayMode: .always))`.
- **Confirmed environment bug, not ours:** the iOS 26.3 Simulator runtime
  ...
  fix.
- **Follow-ups queued, out of scope here:**
  - Restore last-viewed Page/folder on relaunch — deferred to a device-local
    follow-up, same treatment as desktop's slice 04b (Scope, item 10).
  - Feature-folder restructure (`Grid/` / `Pairing/`) once step 7's
    discovery/pairing work lands a second real feature boundary — Impl
    Note 9.
  - The `@Observable` extraction this slice never needed — flag `PageGrid`
