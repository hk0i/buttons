...
1. **Swift model mirror** of `desktop/src/lib/types/button.ts` — `Config`,
   `Profile`, `Page`, `ButtonModel` (see naming note below), and
   `ButtonContent` as an enum with associated values
   (`.actions([Action])` / `.folder([ButtonModel])` / `.back`), plus
...
1. `ios/Buttons/Models/ButtonModel.swift` — create — the Swift model mirror
   ...
   `ButtonModel` / `ButtonContent` / `Action` / `MediaKeyKind`.
...
3. `ios/Buttons/Views/DeckButton.swift` — create — one button cell: icon
   glyph + label, `.back` fallback glyph/label. Also holds `DeckButtonStyle`
   (a `ButtonStyle` that dims the cell while pressed) — kept with the cell
   since it owns that look; the grid applies it. No `View` suffix — SwiftUI
   idiom (`Button`, `Text`, …); the colliding model is `ButtonModel`, not
   this.
4. `ios/Buttons/Views/ButtonGrid.swift` — create — a scrolling `LazyVGrid`
   of `DeckButton` cells for a given `[ButtonModel]`; 4 columns portrait /
   8 landscape via `verticalSizeClass`; wraps each cell in a `Button` styled
   with `DeckButtonStyle` and emits an `onTap(ButtonModel)` callback the
   caller dispatches on `ButtonContent`.
...
becomes **`ButtonModel`** — `Button` is a SwiftUI view, and an
...
struct ButtonModel: Identifiable {   // button.ts `Button` — see Naming
    let id: String
    var label: String?
...
2. **Not a base the proto types will wrap.** EDD §5.2's export decision is
   ...
   wrapper/adapter layer is worse, not better: `.folder([ButtonModel])`
...
1. `label` / `icon` are `String?` here. The nil-vs-empty-string distinction
   ...
   `optional` at step 6.
2. `Profile` / `Page` / `ButtonModel` conform to `Identifiable` here
...
5. **`.back` rendering matches desktop** — desktop's `DeckButton.svelte`
   falls back to `⬅️` and "Back" when icon/label are nil; do the same. The
   `.back` cell is a normal grid cell (no special chevron chrome), which is
   the whole point of it being real button data.
6. **Glyphs must be emoji-presentation.** Text-default codepoints (`⬅`
   U+2B05, the media symbols `⏯⏮⏭`, `↔`, `⌨`, `🖥`) render as monochrome
   text — or tofu — unless followed by U+FE0F (VS16): write `⬅️`, `⏯️`, etc.
   Applies to the hard-coded `.back` fallback and to every icon in
   `MockConfig`.
7. **The iOS 26.3 Simulator runtime cannot render color emoji at all —
   confirmed environment bug, not a code issue.** Every glyph shows a
   `?`-in-box placeholder regardless of font (`plain`, `.system(size:)`,
   `.largeTitle`) or VS16; the same build renders correctly on the iOS 17.4
   Simulator and on a real device (iPhone 14 Pro Max). Reinstalling the
   26.3 runtime (Xcode → Settings → Components) did not fix it. **Do the
   DoD's visual walkthrough on an iOS 17.x Simulator or a real device, not
   `iPhone 17`/26.3** — its placeholder box would otherwise misread as a
   broken `.back`/icon fallback in our own code. The build command still
   targets `iPhone 17` (compiles/runs fine there; only rendering is
   affected). Track upstream; drop this note once Apple ships a fixed
   runtime.
8. Keep the model layer free of SwiftUI imports — plain value types, same
   Tauri-agnostic-core discipline the desktop's `config.rs` follows. This is
   also why the model type is `ButtonModel`, not `Button`: the moment a
...
9. **Folder structure is type-based (`Models/` / `Views/` / `Data/`) on
...
10. **Locking Page-swipe while a folder is open is a data change, not a
    gesture suppression — a gesture-based attempt was tried and doesn't
    work.** `TabView(.page)`'s swipe is an internal UIKit recognizer;
    `.highPriorityGesture(DragGesture(minimumDistance: 20))` on the
    container does not reliably out-prioritize it (confirmed: a short
    swipe still paged through). Dropping `minimumDistance` to catch shorter
    swipes just swallows taps on the grid underneath instead — no threshold
    wins both. Fix: `ContentView` feeds `TabView`'s `ForEach` a
    `visiblePages` array of **one** Page (the current one) while
    `isFolderOpen`, instead of all of them — the pager has nowhere else to
    go, no gesture race. **Risk checked, confirmed safe (device walkthrough,
    2026-09-10):** `PageGrid`'s `folderStack` is `@State`, keyed to its
    Page's view identity — shrinking `ForEach`'s collection to one element
    and back could plausibly have torn down and recreated the surviving
    `PageGrid`, resetting `folderStack`. It doesn't: SwiftUI's `ForEach`
    diffs by id, and the surviving Page's id is unchanged across the
    collection-size change, so its `@State` persists. Entering a folder
    keeps it open. No need for the `@Observable`-lift or hand-rolled-pager
    fallback considered here.

## Definition of Done

1. [x] `xcodegen generate` succeeds and the build command below builds
       clean.
2. [x] App launches in the iOS Simulator showing a grid of mock buttons with
       icons and labels. Confirmed 2026-09-10 (screenshot, iOS 17.4 sim).
3. [x] Swiping left/right moves between the mock Profile's Pages; each Page
       shows its own independent buttons.
4. [x] Tapping a folder button drills into its nested grid, which shows a
       real `Back` cell as the first element (⬅ / "Back") — **and stays
       there** (doesn't snap back to the top-level grid on its own). Confirms
       Impl Note 10's open risk didn't materialize: collapsing `TabView` to
       one visible Page doesn't reset `PageGrid`'s `folderStack`. Confirmed
       2026-09-10 on device.
5. [x] Tapping the `Back` cell returns to the parent grid.
6. [x] A folder nested two levels deep can be navigated all the way in and
       all the way back out via its own `Back` cells.
7. [x] Paging between Pages is disabled while a folder is open, and
       re-enabled after backing out to the Page's top level. Confirmed
       2026-09-10 — this is the swipe-lock bug (Impl Note 10), found and
       fixed, then re-verified working.
8. [x] Rotating to landscape re-lays the grid to 8 columns; rotating back
       returns to 4. No clipped cells, no horizontal scroll bleed.
9. [x] Tapping a regular (`.actions`) button does not crash and does not
       navigate; at most a brief pressed visual.
10. [x] Code check: the Swift model in `ButtonModel.swift` mirrors
        `desktop/src/lib/types/button.ts` — one `ButtonContent` enum, no
        `isFolder` boolean, no parallel optional arrays; `label`/`icon` are
        `String?`; no `Codable`. `button.ts`'s `Button` is `ButtonModel`;
        no view file qualifies `SwiftUI.Button`. Verified 2026-09-10 by
        reading `ButtonModel.swift` and grepping `ios/Buttons/` for
        `SwiftUI.Button` (no matches).
...
```
Then walk every checklist item — on an **iOS 17.x Simulator or a real
device** for anything glyph-visible (Impl Note 7: iOS 26.3 Simulator can't
render color emoji). No automated tests at this stage — same rationale as
slices 2–4: UI/UX and a data-model shape that need eyes on them, not unit
coverage.
...
DoD walked on device (iPhone 14 Pro Max) and iOS 17.4/26.3 Simulators,
2026-09-10; all items pass.

1. **Bug found + fixed during the walkthrough:** initial swipe-lock used
   `.highPriorityGesture(DragGesture(minimumDistance: 20))` on the `TabView`
   — didn't work, a short swipe still paged through. `TabView(.page)`'s swipe
   is an internal UIKit recognizer a SwiftUI gesture can't reliably
   out-prioritize. Fixed by feeding `TabView` a `visiblePages` array of one
   Page while a folder is open, instead of suppressing the gesture — see
   Impl Note 10. Checked the resulting risk (collapsing to one Page resetting
   `PageGrid`'s `@State`) explicitly; it doesn't happen, `ForEach`'s
   id-keyed diffing preserves it.
2. **Bug found + fixed:** page-indicator dots were invisible (unselected-gray
   on transparent, over a white screen). Fixed with
   `.indexViewStyle(.page(backgroundDisplayMode: .always))`.
3. **Confirmed environment bug, not ours:** the iOS 26.3 Simulator runtime
   cannot render color emoji at all (every glyph shows a `?`-in-box,
   regardless of font or VS16) — confirmed after a full runtime reinstall.
   Real device and the iOS 17.4 Simulator both render correctly. See Impl
   Note 7. Do glyph-visible checks on 17.x or a device until Apple ships a
   fix.
4. **Follow-ups queued, out of scope here:**
   1. Restore last-viewed Page/folder on relaunch — deferred to a device-local
      follow-up, same treatment as desktop's slice 04b (Scope, item 10).
   2. Feature-folder restructure (`Grid/` / `Pairing/`) once step 7's
      discovery/pairing work lands a second real feature boundary — Impl
      Note 9.
   3. The `@Observable` extraction this slice never needed — flag `PageGrid`
      as a candidate to re-check once step 7's connection layer exists
      alongside it, per the new `~/.claude/CLAUDE.md` § SwiftUI rule.
