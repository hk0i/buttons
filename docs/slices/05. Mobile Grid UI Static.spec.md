...
6. **Glyphs must be emoji-presentation.** Text-default codepoints (`⬅`
   ...
   text — or tofu in a `Text` whose font lacks the symbol — unless followed
   by U+FE0F (VS16): write `⬅️`, `⏯️`, etc. Applies to the hard-coded `.back`
   fallback and to every icon in `MockConfig`. (Xcode's *preview canvas*
   separately renders even valid color emoji as tofu sometimes — a canvas
   bug; the Simulator is the source of truth.)
7. Keep the model layer free of SwiftUI imports — plain value types, same
...
8. **Folder structure is type-based (`Models/` / `Views/` / `Data/`) on
