...
1. **Swift model mirror** of `desktop/src/lib/types/button.ts` — `Config`,
   `Profile`, `Page`, `Button`, and `ButtonContent` as an enum with
   associated values (`.actions([Action])` / `.folder([Button])` / `.back`),
   plus `Action` and `MediaKeyKind`. Plain value types only. No `Codable`
...
3. **Grid rendering** — a scrollable grid of button cells for the current
   ...
   **4 in portrait, 8 in landscape** (see slice 4's density note — mobile
   ...
   `⬅` / "Back", matching desktop's `DeckButton`.
...
3. `ios/Buttons/Views/DeckButtonView.swift` — create — one button cell:
   icon glyph + label, `.back` fallback glyph/label, pressed-state visual.
4. `ios/Buttons/Views/ButtonGridView.swift` — create — the column grid for a
...
5. `ios/Buttons/Views/PageView.swift` — create — one Page: hosts the folder
   navigation stack (top-level grid + drilled-in folder grids) over that
   Page's buttons.
6. `ios/Buttons/Views/ContentView.swift` — create — the paged Pages
   ...
   old file, create this one. `xcodegen` globs the whole `Buttons` dir so
   the relocation is free, but the git diff is a delete + an add, not a
   rename.)
7. `ios/Buttons/ContentView.swift` — delete — superseded by
   `Views/ContentView.swift` above.
8. `ios/Buttons/ButtonsApp.swift` — modify only if the `ContentView` file
   move requires it; otherwise leave.
9. `ios/Buttons.xcodeproj/project.pbxproj` — regenerated — `xcodegen`
...
2. **Not a base the proto types will wrap.** EDD §5.2's export decision is
   ...
   recursive, so wrapping means a recursive adapter over the generated
...
1. `label` / `icon` are `String?` here. The nil-vs-empty-string distinction
   ...
   `optional` at step 6.
2. Generated proto structs are not `Identifiable`. To keep the step-6 delta
...
4. **Column count is hardcoded 4 / 8**, mirroring
   ...
   not add one here.
5. **`.back` rendering matches desktop** — `DeckButton` falls back to `⬅`
   and "Back" when icon/label are nil; do the same. The `.back` cell is a
   normal grid cell (no special chevron chrome), which is the whole point of
   it being real button data.
