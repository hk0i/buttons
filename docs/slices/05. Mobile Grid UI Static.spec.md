...
`Codable`. This is the second and last UI-only slice: by the end of it both
platforms' real UI needs are known, so step 6 can translate a shape into the
`.proto` schema rather than guess at one.
