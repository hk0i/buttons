...
sequenceDiagram
  ...
  participant Deb as config_sync_debounce task
  ...
  Cmd->>Deb: dirty_tx.send(config) — notify (sibling call, not nested in Cfg)
  Note over Deb: coalesce bursts, 1000ms quiet period, dirty-check
  Deb->>Conn: config_changed_tx.send(()) — signal only, no payload
...
the persist having *completed* in any observable way `Deb`/`Conn` could
...
4. **Persist and notify are two sibling calls made by the `save_config`
   ...
   *and* "something downstream cares" — that's the coordinator, not
   ...
   statements; nothing downstream (`Deb`, `Conn`, mobile) can observe or
...
7. **Why `build_and_send_config_sync` reloads `Config` from disk instead
   ...
   writer (no import/export command touches it), so `Deb`'s `latest` and
