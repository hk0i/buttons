...
4. [x] Opening the editor on an existing button and closing it without
   changing anything produces **no** `config_sync` send (dirty-check,
   Implementation Notes #2). Also the regression check for the
   `last_sent` seeding fix (Interface Note 4) — done as the first editor
   interaction after a fresh launch. Confirmed 2026-09-16 — printed
   `skipped (no-op)`.
