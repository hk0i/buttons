...
**The two callers apply different failure policies to the same `Err(())`** —
the helper's job is to build and send, not decide what a failure means to
its caller:
...
// A failure here — most likely a transient config_path read — skips this
// one resync and keeps the connection alive; dropping a live session over
// one missed resync is worse than mobile staying briefly stale until the
// next edit. See Implementation Notes #8 for the Lagged arm alongside it.
