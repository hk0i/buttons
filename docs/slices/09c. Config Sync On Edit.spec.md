...
3. [ ] **The discriminating coalescing test — plain "type continuously"
   doesn't exercise `QUIET_PERIOD` at all**, since `ButtonEditor.svelte`'s
   own 500ms debounce already restarts on every keystroke and produces
   zero saves until typing pauses. Instead: make 3-4 distinct edits to a
   button (e.g. relabel it 3-4 times), each committed roughly 700ms apart
   — longer than the frontend's 500ms so each one actually reaches
   `save_config`, shorter than the server's 1000ms `QUIET_PERIOD` so the
   debounce task has to coalesce them. Confirm exactly one `config_sync`
   send, not four — via a temporary `eprintln!` at the
   `ConfigChangedTx::send` call, or by counting frames received on a
   `websocat` client standing in for mobile (`protocol/PAIRING.md` §
