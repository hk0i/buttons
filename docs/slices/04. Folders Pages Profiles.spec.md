...
- **Single click vs. double click, a UX decision worth stating plainly:** a
  ...
  all. One dispatch-on-`ButtonContent`-variant behavior, reachable from two
  places: double-clicking the cell in the preview grid, and — new for this
  slice — a "Content" panel inside `ButtonEditor` itself for `Folder`-content
  buttons, replacing where the action-array editor and Test button would
  otherwise sit. Its "Show Content" button calls the exact same
  navigate-into-folder path double-click uses; it exists because the editor
  is where you're already looking after toggling a button to `Folder`, and
  making you leave the editor, find the cell, and double-click it just to
  see what's inside would be a needless detour. Single-click-to-select is
  what makes double-click safe to add on the grid: it can't be confused with
  the editing click since they're different gesture counts, not different
  targets.
