...
0. **Network POC (spike — throwaway, not a shippable slice)** — de-risks the
   ...
   and proves the WebSocket channel works in *both* directions: a standalone
   Rust CLI (no Tauri) that mDNS-advertises, runs a WebSocket server, renders a
   terminal QR code, and — once connected — sends an "enable" message to the
   client; a minimal SwiftUI app (one `Button`, "Send It!", disabled until the
   enable message arrives) that discovers the desktop via `NWBrowser` and, on
   tap, sends a message the desktop logs to its terminal. Lives entirely in
   `/spikes/network-poc/`, deleted once proven — see
