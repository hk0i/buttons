...
mirroring"): authority/direction (bidirectional implies echo suppression +
concurrent-nav conflict handling), the new `.proto` message shape, and how it
interacts with §5.5's independent mobile swipe/folder navigation. Desktop-side
last-viewed-page *persistence* (slice 04b) is separate: device-local
`localStorage`, restored on launch, and does **not** put navigation state into
`Config` or the wire.
...
- **Live navigation-cursor mirroring (page + folder path).** §5.2's revised
  ...
  step 7:
  - **Authority / direction.** Product intent is bidirectional (navigate on
    either device, the other follows). That needs echo suppression (the
    follower must not re-broadcast the position it was just told) and a rule
    for near-simultaneous navigation on both ends (last-writer-wins on a
    monotonic counter is the likely answer — confirm against real latency).
  - **Message shape.** A new `nav_cursor` message `{ pageId: string,
    folderPath: repeated string }` (or fold it into `profile_switch` — but
    they have different fire rates, so probably separate). New `.proto`
    field(s); still free pre-lock (schema locks step 6 — this may need to
    land just after).
  - **Coalescing.** App launch restores Profile then page back-to-back; a
    naive implementation fires two cursor pushes. Same debounce/dirty-check
    machinery §5.2's "don't push a sync unless something changed" corollary
    already defers to step 7 — one more input to that design.
  - **Interaction with independent mobile navigation** (§5.5 lists
  ...
  - Desktop-local last-viewed-page persistence (slice 04b) is **not** this:
...
- **Desktop grid preview accuracy and grid density, tiered.** The desktop
