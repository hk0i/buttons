...
noise. `ContentView` and `PairingView` keep it: bare `Content` carries no
information, and bare `Pairing` is unavailable — `PairingPayload`,
`PairingStore`, and `PairingSession` already occupy that name in the same
module, so a type called just `Pairing` would read as the domain model,
not the screen. Not an exception to a "drop `View`" rule — there was never
a blanket rule, just this one, applied consistently.
