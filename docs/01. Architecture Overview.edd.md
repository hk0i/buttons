...
12. **iOS UI polish & App Store review prep.** Promoted from 11a
    ...
    pass across existing screens, and **all open `07*` follow-ups, folded
    in wholesale 2026-09-23** rather than fixed ad hoc per slice — Apple
    App Store review approval is the actual gate this step exists to clear
    before Android work starts:
    1. `docs/slices/07a. Pairing Scanner Polish.spec.md` — viewfinder
       overlay, dim/blur, scan-instruction placement. Also gains a
       back/cancel affordance out of `.scanning` (and, pending scope
       confirmation, `.cameraPreAsk`/`.cameraDenied` too) — found
       2026-09-23 during 11a's DoD walkthrough: none of the three camera
       states have any way back to `.landing` today except completing or
       denying the flow.
    2. `docs/slices/07b. Desktop Connection Debug Tools.spec.md` —
       desktop-side connection-status/force-disconnect UI, behind a
       persisted debug-tools toggle.
    3. Add a proper top bar
    4. Add an auto-reconnect message on root screen:
