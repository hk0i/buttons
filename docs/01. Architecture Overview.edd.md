...
already holds that Profile's full page/folder tree. What the wire actually
tracks is *which Profile is active*, never a live navigation cursor — see the
still-pending nav-state-sync note for the mobile-UX side of this.
...
- **Profile-switch churn** (rapid OS-focus alt-tabbing) — the debounce on the
  focus watcher (§5.5) already covers this; it now only needs to coalesce
  `profile_switch` events, which are cheap regardless.
- **Edit churn** — Slice 4's `ButtonEditor` autosaves on a 500ms debounce per
