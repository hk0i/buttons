...
9. **Stateful buttons** — desktop `state_push` for toggle-style actions; mobile
   reflects live without a manual refresh. Design settled, Open Questions
   → "Stateful buttons — `state_push` design."
10. **Profile switching, cross-device** — manual switch from mobile
    ...
    auto-switching.
11. **QR reconnect fallback** — on-demand QR redisplay + rescan-to-reconnect, plus
    manual IP entry for cameraless devices.
12. **Second mobile platform (Android)** — port the now-validated flow.
13. **Tier 2 integrations** — OBS control (obs-websocket v5) first, per-app audio
    mixer as a parallel/second slice.
14. **Profile export/import** — plain-JSON file, for community sharing.
15. **Packaging & distribution** — Tauri bundler (dmg/msi/deb/AppImage); App
...
5. **Cross-platform hotkey portability in shared Profiles.** A `Hotkey`
   ...
   (roadmap step 14, PRD's "shareable, open configs") — a `Hotkey` exported on
...
8. **Stateful buttons — `state_push` design.** Settled for step 9. No Tier 1
   use case (§4 Use Cases: launch app / hotkey / media key) produces real
   external state — the PRD's own stateful-button examples (mic mute, OBS
   record) are Tier 2/3, scheduled well after step 9 (step 13). Every real
   use case this flag will ever carry — mic mute, OBS active scene, a
   soundboard sound currently playing, a desktop operation in flight — is
   a **live signal with no authored value**, not something a user sets and
   expects to persist. That shapes every decision below:
   1. **`is_active` is not persisted.** It does not live on `config::Button`
      (the Rust struct serialized to `buttons.json`) and never appears in
      an exported Profile file. Decisive reason, not just taste: PRD Button
      Model 7 makes exported Profiles plain shareable files — a persisted
      flag would ship "the mic happened to be muted at export time" inside
      someone else's shared config. Secondary: a couple of the real use
      cases above (a sound playing, an operation in flight) would mean a
      disk write per flip. Desktop holds current values in an in-memory
      map only (`button_id -> bool`), reset empty on every launch — a
      button with no reporter yet, including right after a restart, is
      false, not stale-true. `bool is_active = 7` still exists on
      `buttons.proto`'s wire `Button` message (below) — the wire schema
      and the on-disk schema are allowed to diverge; `proto.rs`'s
      `From<&config::Button>` conversion is precisely that seam.
   2. **State lives on `Button` itself, not a new `Action` variant:**
      generic like `label`/`icon` — any button can carry it, not only
      `.actions` buttons.
   3. **v1's only source of state changes is a test affordance in the
      desktop `ButtonEditor`** — next to the existing `Test` button (which
      already fires an action rather than configuring one; the parallel is
      deliberate), not a bound checkbox that would misrepresent a
      transient runtime flag as a persisted property. Flags in the slice
      spec as removable/supersedable once a real reporter (Tier 2, step
      13) exists. A future integration writes to the same in-memory map
      through the same broadcast path; the plumbing built here doesn't
      change.
   4. **Wire: new `StatePush` (D→M) message**, `{ button_id, is_active }`,
      next free `Envelope` tag. Unsolicited, matching `state_push`'s table
      row (§5.2) — sent whenever the in-memory map (Note 1) changes,
      whatever the caller.
   5. **Precedence vs. `config_sync`: `config_sync` always wins** — same
      "remove the invariant, don't maintain it" reasoning as §5.2's
      whole-Config sync decision, retargeted at an in-memory source of
      truth instead of disk: a `config_sync` is built by merging the live
      map into a fresh proto `Config` at send time, so it is always
      current by construction. Mobile clears any `state_push` overrides on
      every `config_sync`; no ordering/sequencing guarantee needed between
      the two message types.
   6. **Mobile render:** `isActive` (config_sync's initial value,
      overridden by any later `state_push` for that id) drives
      `DeckButton`'s **background** channel — the channel slice 08 (Out
      #7) reserved for exactly this, composing with the existing
      press-flash icon overlay without a rewrite.
   7. **No sequence-counter wrapper needed** (unlike slice 08's
      `ActionResultEvent`) — `state_push` drives a direct `@Observable`
      property read in the view body, not an `.onChange` side effect/
      timer, so Observation's unconditional-mutation tracking already
      fires on a repeat value.
