...
11. **QR reconnect fallback** — on-demand QR redisplay + rescan-to-reconnect, plus
    ...
    1. (11a) **iOS UI polish & App Store review prep** — gates step 12, not bundled
    with it: real theming via design tokens (replacing the placeholder
    `Color` values `StatusColors.swift` has flagged since slice 09), a UX
    cleanup pass across existing screens, and the camera/QR scanner polish
...
8. **Stateful buttons — Switch button type + `state_push` design.** Settled
   ...
   3. **Current-index persistence is split, deliberately, not "never
      ...
      establishes), loaded at launch, excluded from whatever step 14's
   ...
   6. **Step 9a — `state_push`, the correction channel.** Needed the
      ...
      integration reporting real drift (step 13). New `Envelope` case:
      ```protobuf
      message StatePush {
   ...
   7. **Forward compatibility: reading live state from a real source.**
      Not step 9/9a's problem to solve — noted here so step 13 (or
      ...
      1. **Nothing in 9/9a's design blocks this.** `SwitchState.actions`
         ...
         reporter could use.
      2. **The primitive step 13 needs already gets built in 9a**, if 9a
      ...
      3. **Genuinely undecided, left for step 13:** whether a real
      ...
      4. **Media-key mute ≠ system volume mute ≠ per-app mute** — on
         ...
         step 13, decoupled from everything in 9/9a; no cross-platform
...
9. **Transport encryption — device-identity keypairs, SSH/Noise model, not TLS.**
   ...
   3. **Cross-platform feasibility, not yet a blocker:** iOS `CryptoKit`
      ...
      implementation) or `ed25519-dalek`/`x25519-dalek`; Android (step 12)
      has libsodium/BC bindings. Workable, not exotic — but a real
      protocol-layer integration, not a UI-level slice.
   4. **Deliberately not blocking slice 11.** The PIN-based cameraless
      fallback slice 11 settled on inherits the *already-accepted* QR
      threat model (ephemeral, single-use, short window) — it doesn't
      make this problem worse, so it ships independently. This item is
      the placeholder for whenever hardening past the household-LAN
      threat model becomes an actual goal — no roadmap step assigned yet.
   5. **Not an App Store gate.** Confirmed 2026-09-20: ATS (App Transport
      Security) does apply to `URLSessionWebSocketTask` (same URL Loading
      System as HTTP, so `ws://` is treated like `http://`), but the
      standard, narrowly-scoped `NSAllowsLocalNetworking` Info.plist
      exception — built for exactly this "app controls a local-network
      device" category — covers it without needing this keypair/Noise
      work first. Separate, smaller, near-term gap found while checking
      this: the project currently declares **no** ATS exception at all
      (`ios/project.yml` has `NSLocalNetworkUsageDescription`, the
      unrelated local-network *permission* string, but nothing under
      `NSAppTransportSecurity`) — real on-device connections likely
      already fail today independent of this Open Question, and independent
      of slice 11. Worth its own small fix, not bundled here.
