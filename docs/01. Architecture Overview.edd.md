...
4. **Desktop grid UI, dynamic** — add/remove/reorder, assign a Tier 1 action
   (launch app/hotkey/media key), persist to the JSON config (§5.3). Desktop-only,
   proves the action engine + editing UX end-to-end.
5. **Folders & pages (desktop-only)** — nested grids and swipeable pages, still
...
9. **QR reconnect fallback** — on-demand QR redisplay + rescan-to-reconnect, plus
   manual IP entry for cameraless devices.
10. **Second mobile platform (Android)** — port the now-validated flow.
11. **Tier 2 integrations** — OBS control (obs-websocket v5) first, per-app audio
    mixer as a parallel/second slice.
12. **Packaging & distribution** — Tauri bundler (dmg/msi/deb/AppImage); App
