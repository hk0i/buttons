...
This is independent of the wire-format question above: exports stay proto3
JSON permanently, by requirement, regardless of what the live WebSocket
protocol does. The wire format's own JSON-now/binary-later question is
unaffected and still deferred to real measurement (§ above) — once real
`.proto` shapes exist and real JSON is flowing over the wire, that's the
moment to actually profile binary against it, not before.
...
- Frontend: **Svelte** (decision, not left open — compiles away its runtime
...
**Desktop WebView platform constraint:** `window.prompt()` does not render in
Tauri's WKWebView on macOS (it needs the host to implement a native text-input
panel delegate, which Tauri doesn't wire up by default) — it silently no-ops
rather than erroring, which makes it easy to ship a dead button without
noticing. `window.confirm()` does have a default WKWebView implementation and
works fine. Discovered empirically in Slice 4 (`ProfileSwitcher`/page-rename
UI) and fixed there with an inline text input instead of a blocking dialog —
noted here so the same mistake isn't repeated the next time a component
reaches for `prompt()`/`alert()`. Applies to any future desktop UI work, not
just Slice 4.
...
- **Cross-platform hotkey portability in shared Profiles.** A `Hotkey`
...
- **Desktop grid preview accuracy: real device screen dimensions vs. a
  generic default.** The desktop editor's grid preview (added ahead of
  schedule, alongside slice 3) currently renders against a fixed generic
  phone size (375×812) — its whole purpose is showing an accurate "how many
  buttons fit above the fold" count, which a one-size-fits-all default can't
  really deliver given real device variation (iPhone vs. Android, phone vs.
  tablet). Once a real device connection exists (roadmap step 7+), the
  paired mobile client could report its actual screen dimensions (logical
  points/dp, safe-area insets) — as part of the pairing handshake or a
  dedicated message — and the desktop could size the preview to match the
  real connected device instead of a generic default. Not needed before a
  device can actually connect; noted here so it isn't lost before the
  `pair_response`/`config_sync` message shapes are finalized (§5.2).
- **Action-type picker: dropdown vs. searchable command palette.** The desktop
