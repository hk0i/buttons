...
0. **Network POC (spike — throwaway, not a shippable slice)** — de-risks the
   ...
   `Ping` to the client; a minimal SwiftUI app (one `Button`, disabled until
   that `Ping` arrives) that discovers the desktop via `NWBrowser` and, on tap,
   sends a `Ping` back that the desktop logs to its terminal. Lives entirely in
   `/spikes/network-poc/`, kept in the repo afterward as an unmaintained
