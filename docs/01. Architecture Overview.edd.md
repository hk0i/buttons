...
- **Live navigation-cursor mirroring (page + folder path).** §5.2's revised
  ...
  - **`source: deviceId` — observability, not the loop fix.** Worth adding
    to `nav_request` for logging ("who drove this navigation") and for the
    eventual >2-device case where it *does* become load-bearing. Not needed
    for echo suppression under the model above. Device identity is already
    stable from QR pairing.
