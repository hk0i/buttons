...
8. **Stateful buttons — `state_push` design.** Settled for step 9. No Tier 1
   ...
   record) are Tier 2/3, scheduled well after step 9 (step 13). Decided
   without borrowing that scope:
   1. **State lives on `Button` itself, not a new `Action` variant:**
      `bool is_active = 7` (`buttons.proto`), generic like `label`/`icon` —
      any button can carry it, not only `.actions` buttons.
   2. **v1's source of state changes is the desktop `ButtonEditor`** (the
      existing slice 2-4 UI) gaining a persisted "Active" toggle. Manual,
      not a real Tier 1 trigger — the honest way to demo PRD Button Model
      2 / FR Desktop 8's requirement ("state changes... independent of
      button presses... must still be reflected") when no OS-level action
      self-toggles yet. A future Tier 2 integration (step 13) calls the
      same setter this toggle calls; the plumbing built here doesn't
      change.
   3. **Wire: new `StatePush` (D→M) message**, `{ button_id, is_active }`,
      next free `Envelope` tag. Unsolicited, matching `state_push`'s table
      row (§5.2) — sent whenever the setter behind Note 2 runs, whatever
      the caller.
   4. **Precedence vs. `config_sync`: `config_sync` always wins** — same
      "remove the invariant, don't maintain it" reasoning as §5.2's
      whole-Config sync decision. Mobile clears any `state_push` overrides
      on every `config_sync` (the fresh sync already embeds current
      truth); no ordering/sequencing guarantee needed between the two
      message types.
   5. **Mobile render:** `isActive` (config_sync's initial value,
   ...
   6. **No sequence-counter wrapper needed** (unlike slice 08's
