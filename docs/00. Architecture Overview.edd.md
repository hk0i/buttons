...
`01. Product Requirements.prd.md`; this doc covers how it's built.
...
process is PRD-first, EDD-derived (see `01. Product Requirements.prd.md`); this
...
`URLSessionWebSocketTask` on iOS, `OkHttp`/`ktor-client` on Android. JSON
...
| `config_sync` | D→M | Full grid state: buttons, folders, pages, appearance |
...
- Action engine: button ID → pluggable action, registry-based so new action types
  (Tier 2+: OBS, audio mixer) are additive
- Frontend: CSS Grid button editor — drag-and-drop reordering, folders, swipeable
  pages, live icon/label editing (static or animated) — talking to the Rust
  backend over Tauri's IPC, separate from the mobile WebSocket channel
...
- Grid UI mirroring synced config, adaptive to phone/tablet screen size, with
  folder navigation and swipe-between-pages (SwiftUI `LazyVGrid`, Compose
  `LazyVerticalGrid`)
...
**Observability:** not yet designed — TBD once the desktop skeleton exists.
Minimal footprint expected given this is a small background daemon, not a service
with production on-call needs.
...
3. **Desktop grid UI, dynamic** — add/remove/reorder, assign a Tier 1 action
   (launch app/hotkey/media key), persist to the JSON config (§5.3). Desktop-only,
   proves the action engine + editing UX end-to-end.
4. **Folders & pages (desktop-only)** — nested grids and swipeable pages, still
   before any networking work, extending the already-validated data model.
5. **Cross-device slice 1: discovery → pairing → config sync** — mDNS
...
6. **Cross-device slice 2: button press round trip** — mobile tap → `button_press`
...
7. **Stateful buttons** — desktop `state_push` for toggle-style actions; mobile
   reflects live without a manual refresh.
8. **QR reconnect fallback** — on-demand QR redisplay + rescan-to-reconnect, plus
   manual IP entry for cameraless devices.
9. **Second mobile platform (Android)** — port the now-validated flow.
10. **Tier 2 integrations** — OBS control (obs-websocket v5) first, per-app audio
    mixer as a parallel/second slice.
11. **Packaging & distribution** — Tauri bundler (dmg/msi/deb/AppImage); App
...
See `01. Product Requirements.prd.md` for product research and sourcing.
